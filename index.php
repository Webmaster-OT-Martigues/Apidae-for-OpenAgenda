	...
	?>
		...
		<?php				
			echo '				
				<ol class="breadcrumb hidden-xs" style="font-size: 11px;">
					...
					<li class="active"><a href="car.php" target="_blank">carousel</a></li>					
					...
					<li><a href="api.php?test=oui">Mode Test</a></li>
		...
		?>	
			...
			if ($fichier_config_maj=="OK") { 
				$fichier_config_maj="";
				echo '<div class="alert alert-warning alert-icon">Mise à jour du fichier <a href="pages/config.php" target="_blank">config.php</a> <i class="icon"></i></div>';
			}
...
//									  'description' 	=> 	array('fr'=> substr($descriptifCourt, 0, 200).'...'), /* Champ obligatoire ne pouvant excéder 200 caractères par langue */
	...
	/* CREATION DE L'EVENEMENT ------------------------------------------------------------------------------------------------------- */
								...
								echo '<p style="margin: 10px 10px 10px;"><i>'.$retourfiche->presentation->descriptifCourt->libelleFr.'</i></p>';
								echo '</div>';
						...
						echo '</div>';
							...
							<?php
									...
									if ($dejafait==1) { 	
										...
										echo '<a href="api.php?id_update='.$id_update.$valtest.'" target="_self" class="btn btn-sm btn-alt m-r-5">M.A.J</a>'; 
									}
									...
									if ($dejafait==0) {
										...
										if ($erreur_date>0) {
											echo '<a href="api.php?id_create='.$retourfiche->id.''.$valtest.'" data-trigger="hover" data-toggle="popover" data-placement="top" data-content="Les dates/heures ont été corrigés" title="" data-original-title="Note de la publication" target="_self" class="btn btn-sm pover btn-alt m-r-5">AJOUTER</a>';
										}
										else {
											echo '<a href="api.php?id_create='.$retourfiche->id.''.$valtest.'" data-trigger="hover" data-toggle="popover" data-placement="top" data-content="Aucune erreur détectée." title="" data-original-title="Note de la publication" target="_self" class="btn btn-sm pover btn-alt m-r-5">AJOUTER</a>';
										}
									}
								if (empty($reservation_registration)) {
